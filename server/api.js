...
const routes = {
  ...
  'POST /api/account': (req, res, { body }) => {
    ...
    const hadToken = Boolean(data.railway.token);
    setCredentials(nextUser, nextPass);
    if (password && hadToken) {
      update(d => { d.railway.token = null; d.railway.checkedAt = null; });
    }
    ...
    if (fresh) {
      const stored = load().telegram?.botToken;
      if (stored) telegram.useToken(openSecret(stored, kekFor(fresh)));
    }

    send(res, 200, {
      ok: true,
      railwayTokenCleared: Boolean(password) && hadToken,
