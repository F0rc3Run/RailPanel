...
function migrate(data) {
  ...
  for (const key of ['core', 'nodes', 'domains', 'railway', 'meta', 'setup', 'telegram']) {
    merged[key] = { ...fresh[key], ...(data[key] || {}) };
  }

  /* A panel that predates the setup flag has obviously already been set up —
     someone is running it. Without this, updating an existing deployment
     drops its operator at the first-run screen demanding a Railway token,
     and on the free plan, where no token exists, that is a locked door in
     front of their own panel. */
  if (data.setup?.complete === undefined) {
    const hasHistory =
      (merged.admin && merged.admin.user && merged.admin.user !== 'railpanel') ||
      (merged.core.inbounds || []).length > 0 ||
      (merged.core.clients || []).length > 0 ||
      (merged.nodes.clients || []).length > 0 ||
      Boolean(merged.nodes.remark) ||
      Boolean(merged.domains.node);
    if (hasHistory) {
      merged.setup = { complete: true, at: merged.meta?.createdAt || new Date().toISOString(), migrated: true };
    }
  }
