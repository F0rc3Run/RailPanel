...
export function updateClient(id, input) {
  ...
  update(d => {
    ...
    if (input.expiry !== undefined) t.expiry = expiryFrom(input.expiry, input.tzOffset);
...
export async function resetClientTraffic(id) {
  ...
  const result = update(d => {
    ...
    return { ok: true, wasBlocked };
  });

  // Same reasoning as the node side: a blocked client is absent from Xray's
  // config, so re-enabling it in the store alone changes nothing.
  return { ...result, needsApply: Boolean(result.ok && result.wasBlocked) };
}
